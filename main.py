...
class Trainer():
    ...
    def compute_batch_losses(self, outputs): # outputs = embedding coming from model.
        ...
        video_forward = outputs["audio_forward"]
        video_reverse = outputs["audio_reverse"]
        video = torch.cat([video_forward, video_reverse], dim=0)
    
        text_forward = outputs["text_forward"]
        text_reverse = outputs["text_reverse"]
        text = torch.cat([text_forward, text_reverse], dim=0)

        # contrastive loss
        loss_contrastive = self.contrastive(batch_audio=video, batch_text=text)
    ...
    def train(self,model) -> None:
        ...
        for batch in training_loader:
            ...
            audio_embeddings = clap_wrap.CLAPWrap(model).get_audio_embeddings(audio_b) # x = path maybe

            loss = compute_batch_losses(input_list) # input_list : []
