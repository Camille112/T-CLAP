...
text_embeddings_f = clap_wrap.CLAPWrap(model).get_text_embeddings(y)
...
for batch in tqdm(test_loader):
    ...
    audio_embeddings = clap_wrap.CLAPWrap(model).get_audio_embeddings(audio_a)
    ...
    max_value, max_index = torch.max(similarity, 1)
    print(max_index.item())
    print(max_value.item())

    y_pred = F.softmax(similarity.detach().cpu(), dim=1).numpy()
    max_value = np.amax(y_pred, axis=1)
